...
class PublicationDashboard
{
    ...
    public function data(): array
    {
        ...
        $faculty = [];
        $codes = $master->keys()->merge($details->keys())->merge($byLecturer->keys())->unique();
        foreach ($codes as $code) {
            $old = $master->get($code);
            $detail = $details->get($code);
            $papers = $byLecturer->get($code, collect());
            $latest = $papers->sortByDesc(fn ($r) => (int) $r->year * 100 + (int) $r->month)->first();
            if (! $old && ! $papers->count() && ! str_contains(strtolower($detail->campus ?? ''), 'malang')) {
                continue;
            }
            ...
            foreach (array_keys($snapshots) as $year) {
                ...
                $annual[$year] = ['score' => $this->scoring->calculate($code, $rule, $annualPapers),
            ...
            $faculty[] = ['code' => $code, 'name' => $detail->nama_dosen ?? $old->nama_dosen ?? $latest->fm_author ?? $code,
                'program' => $detail->nama_gugus_binaan ?? $programAliases->get($old->jurusan_dosen ?? '') ?? $old->jurusan_dosen ?? $latest->dept ?? 'Belum tercatat',
                'faculty' => $type, 'faculty_detail' => $detail->tipe_faculty ?? $type,
                'education' => $education, 'rank' => $rank, 'rule' => $rule,
                'annual' => $annual, 'profile_source' => $detail ? 'database_dosen_new' : ($old ? 'database_dosen + Raw' : 'Raw')];
        }
        ...
        $issues = [
            ...
            ['feature' => 'Skor KPI', 'detail' => 'Perhitungan operasional memakai aturan sistem saat ini, bobot penyesuaian Scopus, dan profil master terbaru. Bukan skor historis yang sudah disahkan. Dosen tanpa baris publikasi atau profil matriks lengkap ditampilkan tanpa skor.'],
        ...
        return ['faculty' => $faculty, 'years' => $years, 'default_year' => $latestYear, 'snapshots' => $snapshots,
            'publications' => $selected->map(fn ($r) => collect((array) $r)->only(['kode_dosen', 'request_code', 'title', 'submitted', 'status', 'jenis', 'quartile_jurnal', 'bobot', 'bobot_asli', 'source_title', 'publisher', 'year', 'tanggal_pelaporan', 'notes', 'prodi_kpi'])->all())->values(),
