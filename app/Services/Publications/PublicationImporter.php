...
{
    public function __construct(private RawPublicationReader $reader) {}
    ...
    public function import(string $path, string $filename, int $year, int $month, int $period, bool $dryRun = false): array
    {
        ...
        $summary['unmatched_codes'] = array_values(array_diff(array_unique(array_column($parsed['records'], 'kode_dosen')), $known));
        if ($dryRun) {
            return $summary;
        }
        ...
        return DB::transaction(function () use ($parsed, $summary, $filename, $path, $year, $month, $period) {
