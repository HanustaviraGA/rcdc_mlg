  ...
  OrgChart.prototype = {
    ...
    createNode: function (data) {
      ...
      var $nodeDiv = $('<div' + (opts.draggable ? ' draggable="true"' : '') + (data[opts.nodeId] ? ' id="' + data[opts.nodeId] + '"' : '') + (data.parentId ? ' data-parent="' + data.parentId + '"' : '') + '>')
        ...
        .attr('node-data', JSON.stringify(data.data ?? {}));
      if (opts.nodeTemplate) {
        $nodeDiv.append(opts.nodeTemplate(data));
      } else {
        $nodeDiv.append('<div class="title" style="background-color:' + data.color + '">' + data[opts.nodeTitle] + '</div>')
          .append(typeof opts.nodeContent !== 'undefined' ? '<div class="content" style="border-color:' + data.color + '">' + (data[opts.nodeContent] || '') + '</div>' : '');
      }
      //
